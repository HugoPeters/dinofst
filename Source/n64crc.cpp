...
void N64CRC::UpdateCRC(void* aRomBuff, uint32 aRomSize)
{
    gen_table();

    unsigned int crc[2];
    N64CalcCRC(crc, (uint8*)aRomBuff);

    C_MemoryStream strm(aRomBuff, aRomSize);
    strm.SetEndianSwap(true);
    strm.Seek(C_FileSystem::SeekSet, 0x10);
    strm << crc[0] << crc[1];
}
